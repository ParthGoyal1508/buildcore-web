...
- [ ] T001 Add `@tanstack/react-query` to `package.json` and register a `QueryClientProvider` in
      `app/layout.tsx` (or a new `app/providers.tsx` client wrapper) — research.md §3
- [ ] T002 [P] Add Settings routes, user-facing copy (validation/confirmation messages), and the
...
- [ ] T003 [P] Create `app/ui/access-denied.tsx` (shared component rendered when a route's required
      permission is missing) — contracts/settings-ui.md
- [ ] T004 [P] Create `app/lib/settings-utils.ts` with the pure `computeDocumentTypeFlag(isMandatory,
...
- [ ] T005 Extend `middleware.ts` (introduced by feature 001) to decode the access token's
      ...
      §2, spec FR-020
- [ ] T006 Add a further check in `middleware.ts` restricting `/dashboard/settings/users`
      specifically to the Super Admin or HO User role (spec FR-010), on top of the
      `USER_MANAGEMENT` permission check from T005
- [ ] T007 [P] Create `zod` schemas for `Company`, `Role`, `UserSummary`, `Department`,
...
- [ ] T008 [P] Create `app/ui/settings/company-context.tsx`: `CompanyContext` +
...
- [ ] T009 [P] Create a shared `ResponsiveList` pattern/component (table `hidden md:table` +
...
- [ ] T010 [P] [US1] Add `listCompanies()`, `createCompany()`, `updateCompany()` to
      `app/lib/api/settings.ts` (zod-validated, per contracts/settings-ui.md)
- [ ] T011 [US1] Create `app/ui/settings/company-list.tsx`: react-query-backed list using
      `ResponsiveList` (T009), Edit action per row, Status badge (depends on T001, T009, T010)
- [ ] T012 [US1] Create `app/ui/settings/company-modal.tsx`: one `react-hook-form` instance across
...
- [ ] T013 [US1] Create `app/dashboard/settings/companies/page.tsx` (Server Component, initial
      fetch, renders `CompanyList`) (depends on T011)
- [ ] T014 [US1] Wire `company-modal.tsx`'s duplicate-short-code and malformed-GSTIN/PAN error
      responses to inline, tab-specific error display without clearing other tabs' entered data —
      spec Acceptance Scenario 2/4
- [ ] T015 [US1] Wire the Inactive-company exclusion: any other component's company-selector
...
- [ ] T016 [P] [US2] Add `listRoles()`, `createRole()`, `updateRole()`, `deleteRole()` to
      `app/lib/api/settings.ts`
- [ ] T017 [US2] Create `app/ui/settings/role-list.tsx`: `ResponsiveList`-based list with
...
- [ ] T018 [US2] Create `app/ui/settings/role-modal.tsx`: permission multi-select sourced from the
...
- [ ] T019 [US2] Create `app/dashboard/settings/roles/page.tsx` (depends on T017)
- [ ] T020 [US2] Wire the delete-role confirmation dialog to warn about cascading role-clear on
...
- [ ] T021 [P] [US3] Add `listUsers()`, `updateUser()`, `deleteUser()` to
      `app/lib/api/settings.ts`
- [ ] T022 [US3] Create `app/ui/settings/user-list.tsx`: `ResponsiveList`-based list (Name, Email,
...
- [ ] T023 [US3] Create `app/dashboard/settings/users/page.tsx` (depends on T022)
- [ ] T024 [US3] Surface the backend's 409 (last-active-Super-Admin protection) verbatim as the
...
- [ ] T025 [P] [US4] Add `listDepartments()`, `createDepartment()`, `updateDepartment()`,
      `deleteDepartment()` and the Designation equivalents to `app/lib/api/settings.ts`
- [ ] T026 [P] [US4] Create `app/ui/settings/department-tab.tsx`: `ResponsiveList`-based (T009) list
...
- [ ] T027 [P] [US4] Create `app/ui/settings/designation-tab.tsx`: same pattern as T026
- [ ] T028 [US4] Create `app/dashboard/settings/employee-setup/page.tsx`: wraps its five tab
...
- [ ] T029 [US5] Add `listDocumentTypes()`, `createDocumentType()`, `updateDocumentType()` to
      `app/lib/api/settings.ts` (no delete function — `isActive` toggle only)
- [ ] T030 [US5] Create `app/ui/settings/document-type-tab.tsx`: `ResponsiveList`-based (T009) list
...
- [ ] T031 [US5] Register `document-type-tab.tsx` in `employee-setup/page.tsx` (T028)
...
- [ ] T032 [P] [US6] Add `listShifts()`, `createShift()`, `updateShift()`, `deleteShift()` to
      `app/lib/api/settings.ts`
- [ ] T033 [US6] Create `app/ui/settings/shift-tab.tsx`: `ResponsiveList`-based (T009) list +
      add/edit/delete modal (native `<label>`/`<button>` elements, focus-trapped, per research.md
      §7/§8), 409 (still-referenced) shown inline — spec FR-018, FR-021, FR-024
- [ ] T034 [US6] Register `shift-tab.tsx` in `employee-setup/page.tsx` (T028)
...
- [ ] T035 [US7] Add `getCodeSeries()` to `app/lib/api/settings.ts`
- [ ] T036 [US7] Create `app/ui/settings/code-series-tab.tsx`: read-only display of short code +
      next code, no editable sequence field — spec FR-019
- [ ] T037 [US7] Register `code-series-tab.tsx` in `employee-setup/page.tsx` (T028)
...
- [ ] T038 [P] Run `npm run lint` and `next build`/`tsc --noEmit` across all new/modified files and
      fix any violations
- [ ] T039 [P] Manually verify every screen at a mobile viewport (≤428px) shows the card layout,
      not a horizontally-scrolling table — spec FR-021/SC-005
- [ ] T040 [P] Manually verify every interactive control across all screens is keyboard-operable
      with a visible focus indicator — spec FR-024/SC-008
- [ ] T041 Run the full `quickstart.md` validation scenarios end-to-end against a local environment
      (with `buildcore-api`'s Settings module running) and record results
- [ ] T042 Confirm every failed-save case (client validation and simulated server rejection) leaves
