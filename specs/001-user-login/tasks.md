...
- [X] T001 [buildcore-web] Create `app/lib/config.ts` exporting `API_URL`, moving it out of
      `app/lib/api/client.ts` (Constitution Principle III)
- [X] T002 [P] [buildcore-web] Create `app/lib/constants.ts` with: route paths (`/login`,
...
- [X] T003 [P] [buildcore-web] Update `app/lib/api/client.ts` to import `API_URL` from
...
- [X] T004 [buildcore-api] Add and configure a cookie-parsing/response-cookie mechanism (e.g.
...
- [X] T005 [buildcore-api] Add/confirm `status` (`active`/`deactivated`) and `mustChangePassword`
      fields on the User model + migration (data-model.md "User Account") — needed by US1, US2, US3
- [X] T006 [buildcore-api] Add a Login Attempt tracking table/model: `accountId`,
      `consecutiveFailures`, `lockedUntil` + migration (data-model.md "Login Attempt Record") —
      needed by US2 and US5
- [X] T007 [buildcore-api] Add an Activity Log table/model: `eventType`, `accountId`, `timestamp`,
      `ipAddress` + migration (data-model.md "Activity Log Entry") — needed by US1, US2, US4, US5
- [X] T008 [P] [buildcore-api] Add IP-address-based rate limiting (e.g. Nest throttler guard) on
      the `/auth/*` routes, independent of the per-account lockout (spec FR-016)
- [X] T009 [P] [buildcore-web] Create `app/lib/session.ts`: holds the access token in memory only
      (never `localStorage`/`sessionStorage`), with `getAccessToken()`/`setAccessToken()`/
      `clearSession()` helpers (research.md §2)
- [X] T010 [buildcore-web] Create `middleware.ts` at the repo root: redirect any unauthenticated
      request to `/dashboard/*` to `/login` (research.md §3)
- [X] T011 [P] [buildcore-web] Add `zod` schemas for the `/auth/login` and `/auth/refresh-token`
...
- [X] T012 [US1] [buildcore-api] Update `src/auth/auth.service.ts` `login()` to: verify the
...
- [X] T013 [US1] [buildcore-api] Update `src/auth/auth.controller.ts` `POST /auth/login` to accept
      `{ identifier, password, rememberMe }` and set the refresh token as a response cookie — NOTE:
      superseded by `001-user-login-backend`'s own, more detailed spec/tasks for the actual backend
      implementation (rotation, reuse detection, RBAC guard, audit log, lockout, Super Admin
      exception, admin-reset-password); this repo's `[buildcore-api]`-tagged tasks describe the
      contract this frontend depends on, not a second implementation to build independently
- [X] T013a [P] [US1] [buildcore-web] Change `login-form.tsx`'s identifier field from an
      email-only input (`type="email"`, zod `.email()`) to a plain text "Email or Username" field —
      update the label, placeholder, input `type="text"`, and zod schema (`identifier: z.string()
      .min(1, 'Enter your email or username')` — no email-format check) (spec FR-001, FR-003,
      2026-08-28 clarification)
- [X] T014 [P] [US1] [buildcore-web] Add a show/hide password toggle control to
      `app/ui/login-form.tsx` (spec FR-001)
- [X] T015 [US1] [buildcore-web] Update `login()` in `app/lib/api/auth.ts` to accept `identifier`
      (rename from `email`) alongside `password`, send `rememberMe`, validate the response with the
      T011 schema, and store the access token via `app/lib/session.ts` (not `localStorage`)
- [X] T016 [US1] [buildcore-web] Update `login-form.tsx`'s submit handler to redirect to
...
- [X] T017 [US1] [buildcore-web] Update `app/dashboard/page.tsx` to render "Welcome back, {name}!"
      (from `app/lib/constants.ts`, T002) when arriving fresh from login (spec FR-006)
- [X] T018 [P] [US1] [buildcore-web] Point the "Forgot Password?" link in `login-form.tsx` at
      `/forgot-password` (link only — that flow is a separate, later feature per spec.md scope)
- [X] T018a [US1] [buildcore-web] In `login-form.tsx`'s submit handler, branch on the login
...
- [X] T019 [US2] [buildcore-api] Rewrite the error paths in `src/auth/auth.service.ts` `login()`
      so an unregistered identifier, a wrong password, and a deactivated account ALL throw the same
      `UnauthorizedException('Invalid email or password')` — remove the current
      `NotFoundException`/`BadRequestException` split (spec FR-004, research.md §1) — see T013's
      note: `001-user-login-backend` is the authoritative spec for the actual backend build
- [X] T020 [P] [US2] [buildcore-api] Normalize the identifier lookup (against both email and
      username) in `auth.service.ts` to be case-insensitive and trimmed before comparison (spec
      FR-003)
- [X] T021 [US2] [buildcore-web] Confirm/adjust `login-form.tsx`'s empty-field inline errors
...
- [X] T022 [P] [US2] [buildcore-web] In `app/lib/api/auth.ts` `login()`, map any 401 response to
...
- [X] T023 [US3] [buildcore-api] Set the refresh-token cookie's `Max-Age` to 30 days when
      `rememberMe` was true, otherwise a short session-length default (spec FR-009,
      data-model.md "Session")
- [X] T024 [US3] [buildcore-api] Add re-validation to the auth guard used by protected routes: on
...
- [X] T025 [US3] [buildcore-web] Add the "Remember Me" checkbox to `login-form.tsx` and pass its
      value into `login()` (spec FR-001)
- [X] T026 [US3] [buildcore-web] Implement transparent refresh in `app/lib/session.ts`: on a 401
...
- [X] T027 [P] [US3] [buildcore-web] Verify `credentials: 'include'` (T003) covers the
...
- [X] T028 [US4] [buildcore-api] Implement `POST /auth/logout` in `src/auth/auth.controller.ts` /
...
- [X] T029 [US4] [buildcore-web] Add a `logout()` function to `app/lib/api/auth.ts` that calls
      `POST /auth/logout`
- [X] T030 [US4] [buildcore-web] Update the Sign Out handler in `app/ui/dashboard/sidenav.tsx` to
...
- [X] T031 [US5] [buildcore-api] Implement consecutive-failure counting in
...
- [X] T032 [P] [US5] [buildcore-api] Return `423 Locked` with the lockout message and approximate
...
- [X] T033 [P] [US5] [buildcore-api] Send a lockout-notification email to the account's registered
      address when a lock is triggered (spec FR-015)
- [X] T034 [P] [US5] [buildcore-api] Write Activity Log entries for `login_success`,
...
- [X] T035 [US5] [buildcore-web] Handle the `423` response distinctly in `app/lib/api/auth.ts` /
...
- [X] T037 [P] [buildcore-web] `npm run lint` and `tsc --noEmit` (or `next build`) pass with no
      errors
- [X] T038 [buildcore-api] Equivalent lint/build check passes
...
- [X] T041 [P] [buildcore-web] In `app/login/page.tsx` (or `login-form.tsx`), read an
